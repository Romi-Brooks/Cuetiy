...
# RainYi unified 便携 zip — 需先完成 unified 构建
...
$stage = Join-Path $out 'RainYi-unified-portable'
$zip = Join-Path $out 'RainYi-unified-portable.zip'

if (-not (Test-Path (Join-Path $rel 'rain-yi.exe'))) {
  throw "Missing $rel\rain-yi.exe — run scripts/build-exe-unified.ps1 first"
}
...
Copy-Item (Join-Path $rel 'rain-yi.exe') (Join-Path $stage 'RainYi.exe') -Force
$sidecar = Join-Path $frontend "src-tauri\binaries\rainyi-backend-$triple.exe"
if (-not (Test-Path $sidecar)) { $sidecar = Join-Path $frontend 'src-tauri\binaries\rainyi-backend.exe' }
if (-not (Test-Path $sidecar)) {
  $env:CGO_ENABLED = '0'
  Set-Location $backend
  go build -ldflags '-s -w' -o (Join-Path $stage 'rainyi-backend.exe') ./cmd/main.go
} else {
  Copy-Item $sidecar (Join-Path $stage 'rainyi-backend.exe') -Force
}
